...
std::string domainCategoryToString(DomainCategory cat) {
    switch (cat) {
        case DomainCategory::Streaming:      return "Streaming";
        case DomainCategory::SocialMedia:    return "SocialMedia";
        case DomainCategory::Messaging:      return "Messaging";
        case DomainCategory::DeveloperTools: return "DeveloperTools";
        case DomainCategory::CloudServices: return "CloudServices";
        case DomainCategory::ECommerce:     return "ECommerce";
        case DomainCategory::SearchEngine:  return "SearchEngine";
        case DomainCategory::Other:         return "Other";
        default:                             return "Other";
    }
}

DomainCategory domainToCategory(const std::string& domain) {
    if (domain.empty()) return DomainCategory::Other;

    std::string lower = domain;
    std::transform(lower.begin(), lower.end(), lower.begin(),
                   [](unsigned char c) { return std::tolower(c); });

    // Precedence matters (e.g., aws.amazon.com => CloudServices, not ECommerce)
    // CloudServices
    if (lower.find("aws") != std::string::npos ||
        lower.find("amazonaws") != std::string::npos ||
        lower.find("cloudfront") != std::string::npos) {
        return DomainCategory::CloudServices;
    }
    ...
    if (lower.find("discord") != std::string::npos ||
        ...
        return DomainCategory::Messaging;
    }

    // DeveloperTools
    if (lower.find("github") != std::string::npos) {
        return DomainCategory::DeveloperTools;
    }

    // SearchEngine
    if (lower.find("google") != std::string::npos ||
        lower.find("gstatic") != std::string::npos) {
        return DomainCategory::SearchEngine;
    }

    // ECommerce
    if (lower.find("amazon") != std::string::npos) {
...
AppType sniToAppType(const std::string& sni) {
    ...
    if (lower_sni.find("twitch") != std::string::npos ||
        lower_sni.find("ttv") != std::string::npos) {
    ...
    if (lower_sni.find("stackoverflow") != std::string::npos ||
        lower_sni.find("stackauth") != std::string::npos ||
        lower_sni.find("stackprinter") != std::string::npos) {
