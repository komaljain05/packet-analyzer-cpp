...
bool DPIEngine::processFile(const std::string& input_file,
    ...
    uint64_t total_packets = stats_.total_packets.load();
    uint64_t total_bytes = stats_.total_bytes.load();
    double packets_per_sec = static_cast<double>(total_packets) / elapsed_sec;
    double mb_per_sec = (static_cast<double>(total_bytes) / (1024.0 * 1024.0)) / elapsed_sec;
   ...
   // std::cout << fp_manager_->generateClassificationReport();
    ...
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ Processing Time:    " << std::setw(10) << std::fixed << std::setprecision(2) << elapsed_sec << " sec" << "                    ║\n";
    std::cout << "║ Packets/sec:       " << std::setw(10) << static_cast<uint64_t>(packets_per_sec) << "            " << " ║\n";
    std::cout << "║ MB/sec:            " << std::setw(10) << std::fixed << std::setprecision(2) << mb_per_sec << "         " << " ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n";
