...
void ConnectionTracker::classifyConnection(Connection* conn, AppType app, const std::string& sni) {
    ...
    // Preserve existing behavior: category derived from SNI/domain when available.
    DomainCategory cat = domainToCategory(sni);
    classifyConnection(conn, app, sni, cat);
}

void ConnectionTracker::classifyConnection(Connection* conn, AppType app, const std::string& sni, DomainCategory category) {
    if (!conn) return;

    if (conn->state != ConnectionState::CLASSIFIED) {
        conn->app_type = app;
        conn->sni = sni;
        conn->domain_category = category;
