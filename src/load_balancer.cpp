...
LoadBalancer::~LoadBalancer() {
    // Avoid double-shutdown races; stop() already guards on running_
    stop();
}
...
if (fp_index < 0 || static_cast<size_t>(fp_index) >= fp_queues_.size()) {
    std::cerr << "[LB] Invalid fp_index=" << fp_index
              << " size=" << fp_queues_.size() << "\n";
    continue;
}

// Push to selected FP's queue
// Temporary diagnostics: if mapping is inconsistent, we must fix it (not suppress it)
if (fp_index < 0 || static_cast<size_t>(fp_index) >= fp_queues_.size()) {
    std::cerr << "[LB" << lb_id_ << "] invalid fp_index=" << fp_index
              << " fp_queues_.size=" << fp_queues_.size()
              << " num_fps_=" << num_fps_ << " fp_start_id_=" << fp_start_id_ << "\n";
    continue;
}
fp_queues_[static_cast<size_t>(fp_index)]->push(std::move(*job_opt));
